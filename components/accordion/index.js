export class AccordionComponent {
                ...
                <div id="item-${item.id}" class="accordion-collapse collapse" data-bs-parent="#main-accordion">
                    <div class="accordion-body" style="background: #EBEFF4;">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button class="st_calc-btn px-4"
                                style="width: auto; border-radius: 34px; font-size: 15px; background: #ff4d4d; color: white;"
                                id="delete-card-${item.id}" data-id="${item.id}">
                                Удалить
                            </button>
                        </div>
                    </div>
    ...
    render(data, clickListener, deleteListener) {
        ...
        data.forEach(item => {
            ...
            document
                .getElementById(`delete-card-${item.id}`)
                .addEventListener("click", (e) => deleteListener(item.id))
        })
    }
}